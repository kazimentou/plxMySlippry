...
$parms = array(
	'jquery'		=> !empty($plxPlugin->getParam('jquery')) ? $plxPlugin->getParam('jquery') : true,
	'speed'			=> !empty($plxPlugin->getParam('speed')) ? $plxPlugin->getParam('speed') : '800',
	'transition'	=> !empty($plxPlugin->getParam('transition')) ? $plxPlugin->getParam('transition') : 'fade',
	'maxwidth'		=> !empty($plxPlugin->getParam('maxwidth')) ? $plxPlugin->getParam('maxwidth') : '',
	'openwin'		=> !empty($plxPlugin->getParam('openwin')) ? $plxPlugin->getParam('openwin') : false,
	'pager'			=> !empty($plxPlugin->getParam('pager')) ? $plxPlugin->getParam('pager') : false,
	'controls'		=> !empty($plxPlugin->getParam('controls')) ? $plxPlugin->getParam('controls') : false,
	'adaptiveHeight'	=> !empty($plxPlugin->getParam('adaptiveHeight')) ? $plxPlugin->getParam('adaptiveHeight') : false,
);
?>
<style>
...
<form class="inline-form" action="parametres_plugin.php?p=<?= $plugin ?>" method="post" id="form_plxMySlippry">
	<fieldset>
		<p>
